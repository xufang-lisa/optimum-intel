...
class _OVVideoChatFlashQwenForCausalLM(OVModelForVisualCausalLM):
    ...
    def _import_videochat_module(cls, module_name: str, model_ref):
        model_dir = cls._resolve_videochat_source_dir(model_ref)
    ...
    def _build_hf_method_provider(self, model_save_dir):
        ...
        build_ctx = init_empty_weights() if init_empty_weights is not None else nullcontext()
        with build_ctx:
            try:
                provider = self.auto_model_class.from_config(config, trust_remote_code=True)
            except TypeError:
                provider = self.auto_model_class.from_config(config)
    ...
    def _patch_hf_provider_for_ov_inference(self, provider):
        ...
        class _OVVisionTowerProxy(torch.nn.Module):
            def forward(self, images):
                return ov_model.get_vision_embeddings(images)
        ...
        mm_projector = getattr(original_model, "mm_projector", None) if original_model is not None else None
        ...
        class _OVBackboneProxy(torch.nn.Module):
            ...
            # Keep this surface narrow to avoid leaking meta tensors from the HF init-empty model.
            def __init__(self, projector_proxy):
                super().__init__()
                self.vision_tower = _OVVisionTowerProxy()
                self.mm_projector = projector_proxy
                self.image_newline = getattr(ov_model, "image_newline", None)
                self.frame_newline = getattr(ov_model, "frame_newline", None)

            def get_vision_tower(self):
                return self.vision_tower

            def embed_tokens(self, input_ids):
                return ov_model.get_text_embeddings(input_ids)

        backbone_proxy = _OVBackboneProxy(_OVMMProjectorProxy(mm_projector) if mm_projector is not None else None)
        provider.model = backbone_proxy
        provider.get_model = lambda: backbone_proxy
        provider.get_vision_tower = lambda: backbone_proxy.vision_tower
        ...
        prepare_ctx.config = getattr(provider, "config", None)
        ...
        if hasattr(provider, "encode_video_image") and callable(getattr(provider, "encode_video_image")):
            prepare_ctx.encode_video_image = types.MethodType(provider.encode_video_image.__func__, prepare_ctx)
        if hasattr(provider, "prepare_inputs_labels_for_multimodal") and callable(
            getattr(provider, "prepare_inputs_labels_for_multimodal")
        ):
    ...
    def _get_hf_method_provider(self):
        provider = getattr(self, "_hf_method_provider", None)
        if provider is None:
            raise ValueError("Transformers method provider is not initialized.")
        return provider

    def _call_hf_method(self, method_name, *args, **kwargs):
        provider = self._get_hf_method_provider()
        if method_name == "prepare_inputs_labels_for_multimodal":
            prepare_ctx = getattr(provider, "_ov_prepare_ctx", None)
            if prepare_ctx is not None and hasattr(prepare_ctx, method_name):
                return getattr(prepare_ctx, method_name)(*args, **kwargs)
        method = getattr(provider, method_name, None)
        if not callable(method):
            raise ValueError(f"Method '{method_name}' is missing on transformers provider model.")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._hf_method_provider = self._build_hf_method_provider(model_save_dir=model_save_dir)
    ...
    # Delegates to the original transformers model's prepare_inputs_labels_for_multimodal method.
    def get_multimodal_embeddings(
        self,
        input_ids,
        pixel_values=None,
        ...
        **kwargs,
        ...
        _, position_ids, attention_mask, _, new_input_embeds, _ = self._call_hf_method(
